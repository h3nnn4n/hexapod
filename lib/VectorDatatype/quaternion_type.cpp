...
// Multiplication
quat_t quat_t::operator*(const quat_t &r) {
    quat_t q = {w * r.w - v.dot(r.v), w * r.v + v * r.w + v.cross(r.v)};
    return q;
}
...
// Inner product
float quat_t::inner() { return w * w + v.dot(v); }

// Magnitude
float quat_t::mag() { return sqrt(inner()); }
...
vec3_t quat_t::axisX(const bool TO_GLOBAL) {
    ...
    vec3_t u = {2 * (v.x * v.x + w * w) - 1, 2 * (v.x * v.y + w_vz), 2 * (v.x * v.z - w_vy)};
    return u;
}
...
vec3_t quat_t::axisY(const bool TO_GLOBAL) {
    ...
    vec3_t u = {2 * (v.y * v.x - w_vz), 2 * (v.y * v.y + w * w) - 1, 2 * (v.y * v.z + w_vx)};
    return u;
}
...
vec3_t quat_t::axisZ(const bool TO_GLOBAL) {
    ...
    vec3_t u = {2 * (v.z * v.x + w_vy), 2 * (v.z * v.y - w_vx), 2 * (v.z * v.z + w * w) - 1};
    return u;
}
