...
void vec3_t::set(const int i, float input) {
    switch (i) {
        ...
        case 1: y = input; break;
        case 2: z = input; break;
    }
}
...
// Dot product
float vec3_t::dot(const vec3_t r) { return x * r.x + y * r.y + z * r.z; }

// Cross product
vec3_t vec3_t::cross(const vec3_t r) {
    vec3_t v = {y * r.z - z * r.y, -x * r.z + z * r.x, x * r.y - y * r.x};
    return v;
}

// Magnitude
float vec3_t::mag() { return sqrt(x * x + y * y + z * z); }
