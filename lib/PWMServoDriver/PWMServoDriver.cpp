 ...
 * TwoWire interface
 */
PWMServoDriver::PWMServoDriver() : _i2caddr(PCA9685_I2C_ADDRESS), _i2c(&Wire) {}
 ...
 */
PWMServoDriver::PWMServoDriver(const uint8_t addr) : _i2caddr(addr), _i2c(&Wire) {}
 ...
 *  with
 */
PWMServoDriver::PWMServoDriver(const uint8_t addr, TwoWire &i2c) : _i2caddr(addr), _i2c(&i2c) {}
 ...
 */
void PWMServoDriver::begin(uint8_t prescale) {
    _i2c->begin();
    reset();
    if (prescale) {
        setExtClk(prescale);
 ...
 */
uint8_t PWMServoDriver::getPWM(uint8_t num) {
 ...
 *  @param  off At what point in the 4096-part cycle to turn the PWM output OFF
 */
void PWMServoDriver::setAllPWM(uint16_t on, uint16_t off) {
#ifdef ENABLE_DEBUG_OUTPUT
